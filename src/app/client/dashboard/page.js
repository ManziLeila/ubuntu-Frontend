...
export default function ClientDashboardPage() {
  ...
  const completed  = transfers.filter(tx => tx.status === 'COMPLETED').length;
  const pending    = transfers.filter(tx => ['PENDING','OTP_PENDING','PROCESSING'].includes(tx.status)).length;
  ...
  return (
    <div style={{ maxWidth: 1100 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 28, flexWrap: 'wrap', gap: 16 }}>
        <div>
          <h1 className="db-text-primary" style={{ fontSize: 24, fontWeight: 800, margin: 0 }}>
            {t.dash_welcome}, {user?.name?.split(' ')[0] ?? 'there'}!
          </h1>
          <p className="db-text-secondary" style={{ fontSize: 13, marginTop: 4, marginBottom: 0 }}>{t.dash_overview}</p>
        </div>
      ...
      {!kycApproved && (
        <div style={{ borderRadius: 12, padding: '14px 18px', background: 'rgba(217,119,6,.08)', border: '1px solid rgba(217,119,6,.2)', marginBottom: 24, display: 'flex', alignItems: 'flex-start', gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <p style={{ fontSize: 13, fontWeight: 700, color: '#d97706', margin: '0 0 2px' }}>{t.dash_kyc_title}</p>
            <p className="db-text-secondary" style={{ fontSize: 12, margin: 0 }}>{t.dash_kyc_desc}</p>
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 28 }}>
        {stats.map(({ label, value, icon: Icon, color }) => (
          <div key={label} className="db-card" style={{ padding: '20px 22px', borderRadius: 14, border: '1px solid transparent' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
              <div style={{ width: 36, height: 36, borderRadius: 10, background: `${color}20`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Icon size={18} color={color} />
              </div>
              <span className="db-text-secondary" style={{ fontSize: 12, fontWeight: 500 }}>{label}</span>
            </div>
            <p className="db-text-primary" style={{ fontSize: 22, fontWeight: 800, margin: 0 }}>{value}</p>
          </div>
        ))}
      </div>

      {/* Recent transfers */}
      <div className="db-card" style={{ borderRadius: 16, border: '1px solid transparent', overflow: 'hidden' }}>
        <div className="border-gray-100" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 24px', borderBottom: '1px solid' }}>
          <h2 className="db-text-primary" style={{ fontSize: 15, fontWeight: 700, margin: 0 }}>{t.dash_recent}</h2>
        ...
        {transfers.length === 0 ? (
          <div style={{ padding: '48px 24px', textAlign: 'center' }}>
            <Send size={32} color="rgba(201,168,112,.3)" style={{ marginBottom: 12 }} />
            <p className="db-text-secondary" style={{ fontSize: 13, margin: 0 }}>
              {t.dash_no_transfers}{' '}
          ...
          transfers.map((tx, i) => (
            <Link key={tx.id} href={`/client/transfers/${tx.urc}`} style={{
              ...
              borderBottom: i < transfers.length - 1 ? '1px solid' : 'none',
              transition: 'background .1s',
            }}
              className={i < transfers.length - 1 ? 'border-gray-100' : ''}
              onMouseOver={e => e.currentTarget.style.background = 'rgba(201,168,112,.04)'}
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p className="db-text-primary" style={{ fontSize: 13, fontWeight: 600, margin: '0 0 2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {tx.recipientName ?? tx.recipientMsisdn}
                </p>
                <p className="db-text-secondary" style={{ fontSize: 11, margin: 0 }}>
                  {tx.fromCurrency}→{tx.toCurrency} · {format(new Date(tx.createdAt), 'dd MMM yyyy')}
                </p>
              </div>
              <div style={{ textAlign: 'right' }}>
                <p className="db-text-primary" style={{ fontSize: 13, fontWeight: 700, margin: '0 0 4px' }}>{fmtAmt(tx.sendAmount, tx.fromCurrency)}</p>
