...
export default function TransfersPage() {
  ...
  return (
    <div className="space-y-5">
      <h1 className="text-2xl font-bold text-gray-900">My Transfers</h1>

      <div className="flex flex-col sm:flex-row gap-3">
        <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }}
          placeholder="Search by URC or recipient…"
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>{['URC','Recipient','Sent','Received','Corridor','Status','Date',''].map(h => (
              ...
              <tbody className="divide-y divide-gray-50">
                {transfers.map(tx => (
                  <tr key={tx.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-4 py-3 font-medium text-gray-900">{tx.recipientName ?? tx.recipientMsisdn}</td>
                    <td className="px-4 py-3 text-gray-700">{Number(tx.sendAmount).toLocaleString()} {tx.fromCurrency}</td>
                    <td className="px-4 py-3 text-gray-700">{Number(tx.receiveAmount ?? 0).toLocaleString()} {tx.toCurrency}</td>
                    ...
                    <td className="px-4 py-3 text-gray-400 whitespace-nowrap">{format(new Date(tx.createdAt), 'dd MMM yy')}</td>
                    <td className="px-4 py-3"><Link href={`/client/transfers/${tx.urc}`} className="text-brand-600 hover:text-brand-700 text-xs font-medium">View →</Link></td>
                  </tr>
                ))}
              </tbody>
            </table>
