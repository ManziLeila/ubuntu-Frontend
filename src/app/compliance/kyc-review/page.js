...
export default function ComplianceKycListPage() {
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        {loading ? <div className="flex justify-center py-16"><Spinner size="lg" /></div> : apps.length === 0 ? (
          <p className="py-12 text-center text-sm text-gray-400">No applications in this queue.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>{['Applicant','Email','Country','Risk','Docs','Status','Submitted',''].map(h => (
              ...
              <tbody className="divide-y divide-gray-50">
                {apps.map(a => (
                  <tr key={a.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-500">{a.user?.country ?? '—'}</td>
                    <td className="px-4 py-3 text-gray-700">{a.riskScore ?? '—'}</td>
                    <td className="px-4 py-3 text-gray-500">{a.documents?.length ?? 0}</td>
                    ...
                    <td className="px-4 py-3">
                      <Link href={`/compliance/kyc-review/${a.id}`} className="text-brand-600 hover:text-brand-700 text-xs font-medium">Review →</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
